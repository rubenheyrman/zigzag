...
CCS_MAIN(int argv, char **argc)
{
  ...
  O_addr_type_L1 O_loop_bound_L1[nb_cnt] = {1,6,4,16,1};
  bool O_loop_relevancy_L1[nb_cnt]       = {1,1,1,0,1};
  O_addr_type_L2 O_loop_bound_L2[nb_cnt] = {1,4,4,2,1};
  bool O_loop_relevancy_L2[nb_cnt]       = {1,1,1,0,1};
  O_addr_type_L3 O_loop_bound_L3[nb_cnt] = {1,16,1,1,1};
          ...
          for (int b1=0; b1<B1; b1++){
            for (int k2=0; k2<K2; k2++){
              for (int k1=0; k1<K1; k1++){
                for (int b0=0; b0<B0; b0++){
                  for (int k0=0; k0<K0; k0++){
                    if (*(*(*(*(output_array_hw+b1*B0+b0)+k2*K1*K0+k1*K0+k0)+oy0)+ox0) != 0){
                      O_data_in.data[O_cnt_wr] = *(*(*(*(output_array_hw+b1*B0+b0)+k2*K1*K0+k1*K0+k0)+oy0)+ox0);
                      O_cnt_wr++;
