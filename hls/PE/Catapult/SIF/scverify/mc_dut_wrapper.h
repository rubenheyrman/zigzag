// Generated by ZYANG
#ifndef INCLUDED_CCS_DUT_WRAPPER_H
#define INCLUDED_CCS_DUT_WRAPPER_H

#ifndef SC_USE_STD_STRING
#define SC_USE_STD_STRING
#endif

#include <systemc.h>
#include <mc_simulator_extensions.h>

#ifdef CCS_SYSC
namespace HDL {
#endif
#if defined(CCS_DUT_SYSC)
// alias ccs_DUT_wrapper to namespace enclosure of either cycle or RTL SystemC netlist
namespace
    ccs_design {
#if defined(CCS_DUT_CYCLE)
#include "cycle.cxx"
#else
#if defined(CCS_DUT_RTL)
#include "rtl.cxx"
#endif
#endif
}
typedef
    ccs_design::HDL::top<5,32,1,1,O_addr_type_L1,32,1,1,I_addr_type_L1,32,1,1,W_addr_type_L1,5,288,8,8,O_addr_type_L2,312,8,8,I_addr_type_L2,1728,8,8,W_addr_type_L2,5,8640,8,8,O_addr_type_L3,8640,8,8,I_addr_type_L3,55296,8,8,W_addr_type_L3,O_partial_type,I_type,W_type>
    ccs_DUT_wrapper;

#else

// Create a foreign module wrapper around the HDL
#ifdef VCS_SYSTEMC
// VCS support - ccs_DUT_wrapper is derived from VCS-generated SystemC wrapper around HDL code
class ccs_DUT_wrapper : public TOP_HDL_ENTITY
{
public:
  ccs_DUT_wrapper(const sc_module_name& nm, const char *hdl_name)
  : TOP_HDL_ENTITY(nm)
  {
  // elaborate_foreign_module(hdl_name);
  }

  ~ccs_DUT_wrapper() {}
};

#else
// non VCS simulators - ccs_DUT_wrapper is derived from mc_foreign_module (adding 2nd ctor arg)
class ccs_DUT_wrapper: public mc_foreign_module
{
public:
  // Interface Ports
  sc_in<bool> clk;
  sc_in<sc_logic> rst;
  sc_in<sc_lv<16> > O_data_in_rsc_dat;
  sc_in<sc_logic> O_data_in_rsc_vld;
  sc_out<sc_logic> O_data_in_rsc_rdy;
  sc_out<sc_lv<16> > O_data_out_rsc_dat;
  sc_out<sc_logic> O_data_out_rsc_vld;
  sc_in<sc_logic> O_data_out_rsc_rdy;
  sc_in<sc_lv<16> > I_data_in_rsc_dat;
  sc_in<sc_logic> I_data_in_rsc_vld;
  sc_out<sc_logic> I_data_in_rsc_rdy;
  sc_in<sc_lv<16> > W_data_in_rsc_dat;
  sc_in<sc_logic> W_data_in_rsc_vld;
  sc_out<sc_logic> W_data_in_rsc_rdy;
  sc_in<sc_logic> zero_guard_in_rsc_dat;
  sc_in<sc_logic> zero_guard_in_rsc_vld;
  sc_out<sc_logic> zero_guard_in_rsc_rdy;
  sc_in<sc_lv<485> > layer_instruction_in_rsc_dat;
  sc_in<sc_logic> layer_instruction_in_rsc_vld;
  sc_out<sc_logic> layer_instruction_in_rsc_rdy;
public:
  ccs_DUT_wrapper(const sc_module_name& nm, const char *hdl_name)
  :
    mc_foreign_module(nm, hdl_name), 
    clk("clk"), 
    rst("rst"), 
    O_data_in_rsc_dat("O_data_in_rsc_dat"), 
    O_data_in_rsc_vld("O_data_in_rsc_vld"), 
    O_data_in_rsc_rdy("O_data_in_rsc_rdy"), 
    O_data_out_rsc_dat("O_data_out_rsc_dat"), 
    O_data_out_rsc_vld("O_data_out_rsc_vld"), 
    O_data_out_rsc_rdy("O_data_out_rsc_rdy"), 
    I_data_in_rsc_dat("I_data_in_rsc_dat"), 
    I_data_in_rsc_vld("I_data_in_rsc_vld"), 
    I_data_in_rsc_rdy("I_data_in_rsc_rdy"), 
    W_data_in_rsc_dat("W_data_in_rsc_dat"), 
    W_data_in_rsc_vld("W_data_in_rsc_vld"), 
    W_data_in_rsc_rdy("W_data_in_rsc_rdy"), 
    zero_guard_in_rsc_dat("zero_guard_in_rsc_dat"), 
    zero_guard_in_rsc_vld("zero_guard_in_rsc_vld"), 
    zero_guard_in_rsc_rdy("zero_guard_in_rsc_rdy"), 
    layer_instruction_in_rsc_dat("layer_instruction_in_rsc_dat"), 
    layer_instruction_in_rsc_vld("layer_instruction_in_rsc_vld"), 
    layer_instruction_in_rsc_rdy("layer_instruction_in_rsc_rdy")
  {
    elaborate_foreign_module(hdl_name);
  }

  ~ccs_DUT_wrapper() {}
};
#endif

#endif

#ifdef CCS_SYSC
} // end namespace HDL
#endif
#endif


