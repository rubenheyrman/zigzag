...
#pragma hls_ccore_type combinational
#pragma hls_pipeline_init_interval 1
    void CCS_BLOCK(run)(type loop_bound[nb_counters], type tile_sizes[nb_counters], type &pntr, type &cnt, bool &irrel_at_max, bool &irrel_at_zero, bool &all_at_max, type counter[nb_counters], type tile_bound[nb_counters])
    {   
        #pragma hls_unroll yes
        for (int i=0; i<nb_counters; i++){
          tile_size[i] = tile_sizes[i];
...
public:
  // Constructor
  O_dp_sb() : setup(1), rd_irrel_at_max(1), rd_irrel_at_zero(1), wr_irrel_at_max(1), wr_irrel_at_zero(1), 
                   write_flag_top(0), write_flag_bot(0), zero_guard_top_flag(0), zero_guard(1), read_flag0(0), read_flag1(0), read_flag2(0), wr_pntr(0), rd_pntr(0),
                   vld_zg_pntr(0), data_vld(0), data_zg(1), flags_top(0), flags_bot(0), flags_zero_guard(0), flags_wr_zero_guard(0), skid(0),
                   zg_cnt(0), reuse(0),
                   top_irrel_done(1), bot_irrel_done(1), read_data_top_flag(0)
...
#pragma hls_pipeline_init_interval 1
  void CCS_BLOCK(run)(ac_channel<packedData<type,WORDS_in> > &wr_data_top, ac_channel<packedData<type,WORDS_out> > &rd_data_top,
                      ...
                      ac_channel<memlevelInstr<addr_type, nb_cnt> > &instr_in, ac_channel<bool> &done_out) { 
    ...
    if (!flags_bot[1] && (rd_pntr != wr_pntr || data_vld) && !zero_guard) { // Access MSB bit of the flags 2bit variable. Please note the bit access method [].
      read_flag0 = ((!flags_zero_guard[0] && zg_cnt == 0) || (zg_cnt != 0)) && ((!reuse && rd_pntr == vld_zg_pntr && write_flag_top) || (reuse ||  rd_pntr != vld_zg_pntr));
    } else {
      read_flag0 = false;
    }
    if (!flags_top[1] && read_data_top_flag) { // Access MSB bit of the flags 2bit variable. Please note the bit access method [].
      read_flag1 = true; 
      read_data_top_flag = false;
    } else {
      read_flag1 = false;
    }
    if (!flags_zero_guard[0] && zg_cnt == 0 && ((!flags_bot[1] && !zero_guard) || zero_guard) && (rd_pntr != wr_pntr || data_vld || zero_guard)){ 
      read_flag2 = true;
    } else {
      read_flag2 = false;   
    }
    ...
    if (read_flag0) {
      ...
      } else if (!reuse && rd_pntr == vld_zg_pntr && write_flag_top){
        read_data_bot = write_data_top; 
      }
    }
...
/*#ifndef __SYNTHESIS__
    ...
    if (read_flag0 || (read_flag2 && zero_guard)){
      ...
      if ((read_flag2 && zero_guard) || (read_flag0 && !reuse && rd_pntr == vld_zg_pntr && write_flag_top)){
	write_flag_top = false;
      ...
      rd_pntr_cntInst.run(loop_bound, tile_size, rd_pntr, rd_cnt, rd_irrel_at_max, rd_irrel_at_zero, rd_all_at_max, rd_counter, rd_tile_bound);     
      
      if (rd_all_at_max){// (rd_irrel_at_zero && rd_pntr == vld_zg_pntr && vld_zg_pntr == 0){
        reuse = false;
      }
      data_vld = false;
    }
    ...
    // flag to detect if zero guard flag needs to updated and connected memory levels provide data
    start_flag = !reuse && rd_pntr == vld_zg_pntr && skid_buf_wr_zero_guard.not_empty();
    
    // if data available in zero guard pipeline and no reuse exploitable anymore at this memory level
    if (start_flag){
      if (!skid_buf_wr_zero_guard.peek() && start_flag && !write_flag_top){
	write_flag_top = wr_data_top.nb_read(write_data_top);
        data_zg = false;
    ...
    zero_guard = data_zg && (rd_pntr == vld_zg_pntr);
    
    
    // if partial sums are read out of the memory, the data at that spot is invalidated and needs to be updated
    if (!data_vld){//(wr_pntr != rd_pntr) {
      // to prove the pointer is a multiple of the wordlength of the memory
      wr_pntr = ((wr_pntr/WORDS_in)*WORDS_in);
      
      if ((wr_irrel_at_max && !flags_top[1]) || !wr_irrel_at_max){
        write_flag_bot = wr_data_bot.nb_read(write_data_bot);
      }
      if (write_flag_bot){
        ...
        } else if (!wr_irrel_at_max && write_flag_bot){
          #pragma hls_unroll yes
        ...
        // increment pointer based on wordlength
        wr_cnt = WORDS_in;
        wr_pntr_cntInst.run(loop_bound, tile_size, wr_pntr, wr_cnt, wr_irrel_at_max, wr_irrel_at_zero, wr_all_at_max, wr_counter, wr_tile_bound);
    ...
    done_out.nb_write(wr_all_at_max);
...
private: 
  ...
  fifo<packedData<type,WORDS_out>,3> skid_buf_bot;
...
//#ifndef __SYNTHESIS__
    //if (start_flag)
      //printf("O_data_vld: %d, O_mac_pntr: %d, O_vld_zg_pntr: %d, skid_buf_wr_zero_guard.peek %d\n",
             //O_data_vld, O_mac_pntr, O_vld_zg_pntr, skid_buf_wr_zero_guard.peek());
//#endif
