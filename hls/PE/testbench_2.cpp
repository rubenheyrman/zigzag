...
CCS_MAIN(int argv, char **argc)
{
  ...
  O_addr_type_L2 O_loop_bound_L2[nb_cnt] = {6,2,1,1,1};
  bool O_loop_relevancy_L2[nb_cnt]       = {1,1,1,1,1};
  O_addr_type_L3 O_loop_bound_L3[nb_cnt] = {6,2,1,1,1};
  bool O_loop_relevancy_L3[nb_cnt]       = {1,1,1,1,1};

  I_addr_type_L1 I_loop_bound_L1[nb_cnt] = {3,3,3,1,1};
  bool I_loop_relevancy_L1[nb_cnt]       = {1,1,1,1,1};
  I_addr_type_L2 I_loop_bound_L2[nb_cnt] = {6,2,1,1,1};
  bool I_loop_relevancy_L2[nb_cnt]       = {1,1,1,1,1};
  I_addr_type_L3 I_loop_bound_L3[nb_cnt] = {6,2,1,1,1};
  ...
  W_addr_type_L2 W_loop_bound_L2[nb_cnt] = {6,2,1,1,1};
  bool W_loop_relevancy_L2[nb_cnt]       = {1,1,1,1,1};
  W_addr_type_L3 W_loop_bound_L3[nb_cnt] = {6,2,1,1,1};
  ...
  ac_channel<packedData<W_type,mem_words_W_L1_in> > W_wr_data;
  ac_channel<packedData<I_type,mem_words_I_L1_in> > I_wr_data;
  ac_channel<packedData<O_final_type,mem_words_O_L1_in> > O_wr_data;
  ac_channel<packedData<O_final_type,mem_words_O_L1_out> > O_rd_data;
            ...
            for (int fx0=0; fx0<FX0; fx0++){
              for (int c1=0; c1<C1; c1++){
                for (int k0=0; k0<K0; k0++){
                  for (int ox0=0; ox0<OX0; ox0++){
                    for (int oy0=0; oy0<OY0; oy0++){
                      //cout << "OUT: " << (*(*(*(*(output_array_hw+b0)+k2*K1*K0+k1*K0+k0)+oy0)+ox0)) << endl;
                      if (*(*(*(*(output_array_hw+b0)+k2*K1*K0+k1*K0+k0)+oy0)+ox0) != 0){
                        O_data_in.data[0] = *(*(*(*(output_array_hw+b0)+k2*K1*K0+k1*K0+k0)+oy0)+ox0);
...
//b0, c3*C2*C1*C0+c2*C1*C0+c1*C0+c0, oy0+fy0, ox0+fx0);
                      ...
                      while (!HWdone){
                        ...
                        if (O_rd_data.available(1)){
                          ...
                          *(*(*(*(output_array_hw+b0)+k2*K1*K0+k1*K0+k0)+oy0)+ox0) = O_data_out.data[0];
                        }
                      }
//cout << "OFMAPhw " << *(*(*(*(output_array_hw+b0)+k2*K1*K0+k1*K0+k0)+oy0)+ox0) << " | ";
//cout << "OFMAPsw " << *(*(*(*(output_array_sw+b0)+k2*K1*K0+k1*K0+k0)+oy0)+ox0) << " [iteration = " << its << " of " << B*K*C*OY*OX*FY*FX << "]" << endl;
                    }
                  }
                }
              }
