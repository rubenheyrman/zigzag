#ifndef __INCLUDED_sram_128_1296_freepdk45_trans_rsc_H__
#define __INCLUDED_sram_128_1296_freepdk45_trans_rsc_H__
...
template < 
  ...
  ,int ADDR_WIDTH
>
class sram_128_1296_freepdk45_trans_rsc : public mc_wire_trans_rsc_base<128,1296>
{
public:
  sc_in< bool >   clk0;
  sc_in< sc_logic >   csb0;
  ...
  sc_in< sc_logic >   csb1;
  sc_in< sc_logic >   web1;
  sc_in< sc_lv<ADDR_WIDTH> >   addr1;
  sc_in< sc_lv<DATA_WIDTH> >   din1;
  sc_out< sc_lv<DATA_WIDTH> >   dout1;

  typedef mc_wire_trans_rsc_base<128,1296> base;
  MC_EXPOSE_NAMES_OF_BASE(base);

  SC_HAS_PROCESS( sram_128_1296_freepdk45_trans_rsc );
  sram_128_1296_freepdk45_trans_rsc(const sc_module_name& name, bool phase, double clk_skew_delay=0.0)
  ...
  virtual void start_of_simulation() {
    ...
    if (base::_holdtime > 0) {
      ...
      msg << "sram_128_1296_freepdk45_trans_rsc CLASS_STARTUP - CLK_SKEW_DELAY = "
...
private:
  ...
  void clk_skew_delay() {
    ...
    if ( _is_connected_port_0 && (_csb0==0) && (_csb1==0) && (_web0==0)) {
      ...
      if (_w_addr_port_0 >= 0)
        inject_value(_w_addr_port_0, 0, 128, _din0, 0);
    }
    ...
    if ( _is_connected_port_1 && (_csb0==0) && (_csb1==0) && (_web1==0)) {
      _w_addr_port_1 = get_addr(_addr1, "addr1");
      if (_w_addr_port_1 >= 0)
        inject_value(_w_addr_port_1, 0, 128, _din1, 0);
    }
    ...
    if ((_csb0==0) && (_csb1==0)) {
      ...
      if (addr >= 0)
      {
        if (addr==_w_addr_port_0 || addr==_w_addr_port_1) {
          sc_lv<128> dc; // X
          _dout0 = dc;
        }
        else
          extract_value(addr, 0, 128, _dout0, 0);
      }
      else { 
        sc_lv<128> dc; // X
        _dout0 = dc;
    ...
    if ((_csb0==0) && (_csb1==0)) {
      ...
      if (addr >= 0)
      {
        if (addr==_w_addr_port_0 || addr==_w_addr_port_1) {
          ...
          extract_value(addr, 0, 128, _dout1, 0);
      }
      else { 
        sc_lv<128> dc; // X
        _dout1 = dc;
  ...
  int get_addr(const sc_lv<ADDR_WIDTH>& addr, const char* pin_name) {
    if (addr.is_01()) {
      ...
      if (cur_addr < 0 || cur_addr >= 1296) {
#ifdef CCS_SYSC_DEBUG
        std::ostringstream msg;
        msg << "Invalid address '" << cur_addr << "' out of range [0:" << 1296-1 << "]";
  ...
  void reset_memory() {
    ...
    _csb1 = SC_LOGIC_X;
    _web1 = SC_LOGIC_X;
    _addr1 = sc_lv<ADDR_WIDTH>();
    _din1 = sc_lv<DATA_WIDTH>();
    _is_connected_port_0 = true;
    _is_connected_port_0_messaged = false;
    _is_connected_port_1 = true;
    _is_connected_port_1_messaged = false;
  ...
  sc_logic _csb1;
  sc_logic _web1;
  sc_lv<ADDR_WIDTH>  _addr1;
  sc_lv<DATA_WIDTH>  _din1;
  sc_lv<DATA_WIDTH>  _dout1;
  bool _is_connected_port_0;
  bool _is_connected_port_0_messaged;
  bool _is_connected_port_1;
  bool _is_connected_port_1_messaged;
};
#endif // ifndef __INCLUDED_sram_128_1296_freepdk45_trans_rsc_H__
